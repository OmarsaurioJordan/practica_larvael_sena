...
@section('content')
	...
	<form id='form' action="{{ route('categorias.update', $datos->id) }}" method='POST'>
		...
		<div class='row'>
			<div class='col-md-4'>
				<input type='text' name='nombre' class='form-control' placeholder='escriba nombre de categoría' value="{{ old('nombre', $datos->nombre) }}">
				@error('nombre')
        ...
        <div class='row'>
			<div class='col-md-4'>
				<input type='text' name='descripcion' class='form-control' placeholder='escriba descripción de categoría' value="{{ old('descripcion', $datos->descripcion) }}">
				@error('descripcion')
					<div class='error compacto col-lg-5'>{{ $message }}</div>
				@enderror
			</div>
